...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption(option =>
      option
        ...
        .setDescription('Forcer le rafraîchissement')
    ),
  ...
  async displayLeaderboard(interaction, type, page = 1, forceRefresh = false) {
    try {
      ...
      console.log(`[LEADERBOARD] Affichage du classement - Type: ${type}, Page: ${page}, GuildID: ${guildId}`);
      ...
      if (!leaderboard) {
        ...
        if (type === 'money') {
          console.log(`[LEADERBOARD] Recherche des utilisateurs avec guildId: ${guildId} (type: ${typeof guildId}) dans la collection Currency`);
          console.log(`[LEADERBOARD] Vérification du type de guildId:`, guildId, 'type:', typeof guildId);
          
          // Vérifier la connexion à la base de données
          const dbStatus = mongoose.connection.readyState;
          console.log(`[LEADERBOARD] Statut de la connexion MongoDB: ${dbStatus} (0=disconnected, 1=connected, 2=connecting, 3=disconnecting)`);
          ...
          try {
            ...
            console.log(`[LEADERBOARD] Collections disponibles:`, collections.map(c => c.name));
            ...
            console.log(`[LEADERBOARD] Exemple de documents (sans filtre):`, JSON.stringify(allDocsSample, null, 2));
            
            // Vérifier avec le guildId spécifique (avec requête brute)
            const totalUsers = await Currency.countDocuments({ guildId: guildId });
            console.log(`[LEADERBOARD] Nombre d'utilisateurs avec guildId=${guildId}: ${totalUsers}`);
            ...
            console.log(`[LEADERBOARD] Requête directe (collection):`, JSON.stringify(directQuery, null, 2));
            ...
            console.error('[LEADERBOARD] Erreur lors de la vérification de la base de données:', error);
            throw error;
          }
          ...
          console.log(`[LEADERBOARD] Récupération des utilisateurs - skip: ${skip}, limit: ${ITEMS_PER_PAGE}`);
          ...
          console.log(`[LEADERBOARD] Exemples de documents correspondants:`, JSON.stringify(sample, null, 2));
          
          // Exécuter la requête finale avec la collection directement
          const topUsers = await collection.find({ guildId })
            .sort({ balance: -1 })
            .skip(skip)
      ...
      if (!leaderboard.users || leaderboard.users.length === 0) {
        console.log(`[DEBUG] Aucun utilisateur trouvé dans le classement. Vérification des collections...`);
        
        // Vérifier les collections existantes
        const collections = await mongoose.connection.db.listCollections().toArray();
        console.log('[DEBUG] Collections disponibles:', collections.map(c => c.name));
        
        // Vérifier les documents dans Currency
        const allCurrencies = await Currency.find({}).limit(5).lean();
        console.log('[DEBUG] Exemple de documents dans Currency:', JSON.stringify(allCurrencies, null, 2));
      ...
      // Créer l'embed
      const title = leaderboard.type === 'money' 
